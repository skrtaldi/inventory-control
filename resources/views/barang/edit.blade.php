...
<form action='{{ url('admin/'.$data->kode) }}' method='post'>
    ...
    <div class="my-3 p-3 bg-body rounded shadow-sm">
        ...
        <div class="mb-3 row">
            ...
            <div class="col-sm-10">
                <select name="kategori_kode" class="form-control">
                    <option value="" class="text-center">--- Pilih ---</option>
                    @foreach ($kategori as $item)
                        <option value="{{ $item->kode }}" class="text-center" {{ old('kategori_kode', $data->kategori_kode) == $item->kode ? 'selected' : '' }}>
                            {{ $item->nama_kategori }}
                        </option>
                    @endforeach
                </select>
